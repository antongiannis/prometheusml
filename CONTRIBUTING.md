...
- Join the AI ethics discussion on [placeholder...]()
- Test PrometheusML

### AI Ethics discussion
At yaiLab we **democratise** AI and believe in responsible AI use. Whilst we are super excited about AI’s 
...
We need the community's help to ... Join our Discord channel on ... and join the discussion to shape... 
...
contribute to UI, please first come to our [Discord channel ...]() for a quick chat.
...
to automatically create the environment with just several commands. yaiLab provides a `docker-compose.yml` file 
...
# Or build the images by passing your GitLab username to use your own fork
docker compose build --build-arg gl_username=<gitlab_username>
```
...
To check a file with `black` against its style definition:
...
checks like `black`, `flake8`, `isort`, and more using [pre-commit hooks](https://pre-commit.com/). Any warnings from 
these checks will cause the Continuous Integration to fail; therefore, it is helpful to run the check yourself before 
submitting code. This can be done by installing pre-commit:

```sh
pip install pre-commit
```

You can use `pre-commit` to run its checks to the files you have modified with:
```sh 
